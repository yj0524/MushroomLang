...
void operation(char *text, FILE *output) {
    ...
    if(strstt(text, "포자") && stridx(text, "?") != -1) {
    ...
    } else if(strstt(text, "섯")) {
        fprintf(output, "JUN(");
        value(text + 3, output, 0);
        fprintf(output, ")");
    } else if(strstt(text, "브섯!")) {
        fprintf(output, "exit(");
...
int parse(char *text, FILE *output) {
    ...
    if(strcmp(lines[0], "어떻게") || strcmp(lines[nc-1], "이 균이냐ㅋㅋ")) {
        freeLines(lines);
        return 1;
    }
    fprintf(output,
    ...
    for(i=1; strcmp(lines[i], "이 균이냐ㅋㅋ"); i++) {
#ifdef _WIN32
